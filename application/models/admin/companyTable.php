<?php
	 ...
	 */
	$aColumns = array('id', 'name', 'description', 'siteUserEmail', 'categoryName', 'moderated', 'accepted','citydescription');
...
if ( isset($input['sSearch']) && $input['sSearch'] != "" ) {
    ...
    for ( $i=0 ; $i<$iColumnCount ; $i++ ) {
        if ( isset($input['bSearchable_'.$i]) && $input['bSearchable_'.$i] == 'true' && $aColumns[$i] == 'name') {
...
*/
$sQuery = "SELECT SQL_CALC_FOUND_ROWS `company`.*, GROUP_CONCAT(`companycitydescription`.`description`) AS citydescription, `siteuser`.`email` AS `siteUserEmail`, `category`.`name` AS `categoryName`
    FROM `company`
    LEFT JOIN `siteuser` ON `siteuser`.`id` = `company`.`siteUserId`
    LEFT JOIN `category` ON `category`.`id` = `company`.`categoryId`
    LEFT JOIN `companycitydescription` ON `company`.`id` = `companycitydescription`.`CompanyId`
...
foreach ($output['aaData'] as $key => $record){
        //print_r($record);
   $output['aaData'][$key]['symbols'] = mb_strlen(trim($record['7']),'UTF-8')+mb_strlen(trim($record['1']),'UTF-8');
   $output['aaData'][$key]['symbolsHTML'] = mb_strlen(trim(strip_tags($record['7'])),'UTF-8')+mb_strlen(trim($record['1']),'UTF-8');
   $output['aaData'][$key][7]= '';
   if($output['aaData'][$key][6]==0)
        $output['aaData'][$key][7].='<a href="#" onClick="publishCompany(\''.$output['aaData'][$key][0].'\');return false;" class="btn btn-outline green btn-sm">Опубликовать</a>';
   ...
   <a href="/admin/companyDiscount/'.$output['aaData'][$key][0].'/0/add" target="_blank" class="btn btn-outline yellow btn-sm"><i class="fa fa-pencil-square-o"></i> Добавить акцию</a>
